...
__author__ = 'Marc Bounder and Anna Cuomo ?'
__date__ = '?'
__version__ = '0.0.1'
...
def minimal_qtl_processing(QTL_Dir, OutputDir, writeToOneFile=True, compressed = False, overWrite=True, minimalPValue = 1, minimalFeaturePValue = 1, topMode = False, debugMode = False):
    ...
    for file in h5FilesToProcess :
        ...
        partTmp = file.split('_qtl_results_')[1].replace(".h5","")
        partTmp2 = file.split('/')[1].split('_qtl_results_')[0]
        if(debugMode):
            print(partTmp)
        if(writeToOneFile):
            outputFile = f"{OutputDir}/{output_file}all.txt"
        else:
            outputFile = f"{OutputDir}/{output_file}{partTmp}.txt"
        ...
        print(partTmp)
        metadata_path=QTL_Dir+f"/{partTmp2}_"+feature_metadata_file+partTmp+".txt"
        snpmetadata_path=f"{QTL_Dir}/{partTmp2}_"+snp_metadata_file+partTmp+".txt"
        if not os.path.isfile(snpmetadata_path):
        ...
        try :
            ...
            ffea= pd.read_table(metadata_path, sep='\t')
            fsnp= pd.read_table(snpmetadata_path, sep='\t')
        except:
        ...
        try:
            ...
            print(f'failed with - lets remove it and rerun job {partTmp}')
            
            
            os.system(f"rm -r {QTL_Dir}*{partTmp}*")
            continue
