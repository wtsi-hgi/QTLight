...
process OPTIMISE_PCS{
    ...
    script:
        ...
        """  
          ...
          # Try both with and without 'proc_' prefix
          for prefix in "proc_" ""; do
            src="${condition}_${interaction}_symlink/\${prefix}\${var}pcs__${outpath_end}"
            if [[ -d "\$src" ]]; then
              cp -r "\$src"/* ${tensor_input_path}
              break
            fi
          done  
