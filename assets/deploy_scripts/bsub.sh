...
if ["$varname" = ''];
    then
        ...
        echo eqtl will fetch the containers and place them in $varname
        export NXF_SINGULARITY_CACHEDIR=$varname
fi
sample="$RUN_ID"
echo -e "\n Submitting eqtl (https://github.com/wtsi-hgi/eqtl) with input file $INPUT_FILE"
bsub -R'select[mem>8000] rusage[mem=8000]' -J $sample -n 1 -M 8000 -o $sample.o -e $sample.e -q long bash /software/hgi/pipelines/eqtl/assets/deploy_scripts/nohup_start_nextflow_lsf.sh $INPUT_FILE
